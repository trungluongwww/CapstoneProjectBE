...
const create = () => {
  ...
  return [
    ...
    body("rentPerMonth").isInt({ min: 0, max: 100000000 }).notEmpty().withMessage(errorCode.room.ROOM_INVALID_PRICE),
    body("deposit").isInt({ min: 0, max: 100000000 }).notEmpty().withMessage(errorCode.room.ROOM_INVALID_DEPOSIT),
    body("squareMetre")
      .isInt({
        min: 0,
        max: 1000,
